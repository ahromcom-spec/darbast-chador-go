...
import { PackageCheck, Eye, Search, MapPin, Phone, User, Calendar, RefreshCw, ArrowLeftRight, Users, Archive } from 'lucide-react';
...
export default function ExecutiveStageCollected() {
  ...
  const handleStageUpdate = async (orderId: string, newStage: string, orderCode: string) => {
    ...
    try {
      const { error } = await supabase
        .from('projects_v3')
        .update({ 
          execution_stage: newStage as any,
        ...
        {filteredOrders.length === 0 ? (
          <Card>
            <CardContent className="p-12 text-center text-muted-foreground">
              <PackageCheck className="h-12 w-12 mx-auto mb-4 opacity-40" />
          ...
          {selectedOrder && (
            <div className="space-y-4 py-4">
              ...
              <ProgressMediaUploader
                projectId={selectedOrder.id}
                stage={"collected" as any}
                stageName="جمع‌آوری شد"
              />
            </div>
          )}
        </DialogContent>
