...
const dimensionsSchema = z.object({
  length: z.number().min(0.1, {
    message: 'طول باید حداقل 0.1 متر باشد'
  }),
  width: z.number().min(0.1, {
    message: 'عرض باید حداقل 0.1 متر باشد'
  }),
  height: z.number().min(0.1, {
    message: 'ارتفاع باید حداقل 0.1 متر باشد'
  })
});
export default function ScaffoldingForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const {
        ...
        location_coordinates: projectLocation?.coordinates ? `(${projectLocation.coordinates[0]},${projectLocation.coordinates[1]})` : null,
  ...
  if (submitted && requestData) {
    return <div className="bg-gradient-to-br from-background via-secondary/30 to-background min-h-screen pt-4 md:pt-8">
  ...
  return <div className="bg-gradient-to-br from-background via-secondary/30 to-background min-h-screen pt-4 md:pt-8">
      <div className="container mx-auto px-3 md:px-4 py-4 md:py-8">
        <div className="max-w-2xl mx-auto space-y-4 md:space-y-6">
          {/* Back Button */}
          <Button onClick={handleBack} variant="outline" className="gap-2 mb-4 md:mb-6 text-sm">
          ...
          <Card className="shadow-elegant">
            <CardHeader className="text-center pb-3 md:pb-4 p-4 md:p-6">
              ...
              <Badge variant={getTypeBadgeVariant(type)} className="mx-auto text-xs md:text-sm">
              ...
              <CardContent className="space-y-4 md:space-y-6 p-3 md:p-6">
                <div className="grid gap-4 md:gap-6 sm:grid-cols-3">
                  <div className="space-y-2">
                    ...
                    <Input id="length" type="number" step="0.01" min="0.1" placeholder="مثال: 12.5" value={length} onChange={e => setLength(e.target.value)} className={errors.length ? 'border-destructive' : ''} disabled={!isLoaded} required />
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="width" type="number" step="0.01" min="0.1" placeholder="مثال: 8.0" value={width} onChange={e => setWidth(e.target.value)} className={errors.width ? 'border-destructive' : ''} disabled={!isLoaded} required />
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="height" type="number" step="0.01" min="0.1" placeholder="مثال: 3.5" value={height} onChange={e => setHeight(e.target.value)} className={errors.height ? 'border-destructive' : ''} disabled={!isLoaded} required />
                    {errors.height && <p className="text-sm text-destructive">{errors.height}</p>}
                  </div>
                </div>

                {/* Volume Preview */}
                {length && width && height && <div className="p-3 md:p-4 bg-secondary/50 rounded-lg border-2 border-construction/20">
                ...
                <div className="flex flex-col sm:flex-row gap-2 md:gap-3 pt-3 md:pt-4">
                  <Button type="button" onClick={handleBack} variant="outline" className="flex-1 text-sm">
                    انصراف
                  </Button>
                  <Button type="submit" className="flex-1 construction-gradient hover:opacity-90 text-sm" disabled={loading}>
