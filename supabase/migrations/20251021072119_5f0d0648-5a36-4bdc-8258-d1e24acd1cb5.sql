-- DROP old function + rebuild with sequence-based code generation (no retry loops)

DROP FUNCTION IF EXISTS public.create_project_v3(uuid, uuid, uuid, uuid, uuid, text, text, jsonb);
...
-- 3) Align sequence current value with existing numeric-like codes
DO $$
DECLARE
  v_max_num BIGINT;
BEGIN
...
-- 4) Helper function: next_project_code (7 digits)
...
-- Returns a SETOF projects_v3 (same as before)
...
BEGIN
  ...
  -- Generate collision-free code via sequence (no loop needed!)
