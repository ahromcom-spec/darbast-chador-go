-- Fix invalid enum cast by removing references to non-existent role in notify_new_order
-- Replace function to only notify existing roles and the customer
...
BEGIN
  ...
  IF NEW.status = 'pending' AND (TG_OP = 'INSERT' OR OLD.status != 'pending') AND subcategory_code = '10' THEN
    ...
    -- Notify customer with direct link
    IF customer_user_id IS NOT NULL THEN
      PERFORM public.send_notification(
        customer_user_id,
        'سفارش ' || NEW.code || ' ثبت شد',
        'سفارش شما با کد ' || NEW.code || ' برای ' || COALESCE(subcategory_name, 'خدمات داربست با اجناس') || ' با موفقیت ثبت شد و در حال بررسی توسط مدیریت است.',
        '/orders/' || NEW.id,
        'success'
      );
    END IF;
    ...
    -- Notify sales manager (single valid role)
    PERFORM public.notify_role(
...
-- Cleanup any lingering approvals with the invalid role label (safe no-op if none)
DELETE FROM public.order_approvals WHERE approver_role = 'sales_manager_scaffold_execution_with_materials';