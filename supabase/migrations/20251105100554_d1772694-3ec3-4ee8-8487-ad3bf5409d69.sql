...
-- 1) حذف تریگرهای تکراری که ممکن است تداخل ایجاد کنند (با CASCADE)
DROP TRIGGER IF EXISTS trg_ensure_sales_materials_approvals ON public.projects_v3 CASCADE;
DROP FUNCTION IF EXISTS public.ensure_sales_materials_approvals() CASCADE;

DROP TRIGGER IF EXISTS notify_managers_on_new_order ON public.projects_v3 CASCADE;
DROP FUNCTION IF EXISTS public.notify_managers_on_new_order() CASCADE;
